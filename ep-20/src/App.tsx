import './App.css'
import React, {DetailedHTMLProps, FC, PropsWithChildren, ReactNode, useCallback, useRef} from "react";
import useTodos, {ITodo} from "./hooks/useTodos";


//   detailed html to react props are ItrinsicElements attributes
function TodoList<T>({items, render}:
                         PropsWithChildren<React.DetailedHTMLProps<React.HTMLAttributes<HTMLUListElement>, HTMLUListElement>>
                         & {
                         items: T[];
                         render: (item: T) => ReactNode;
                     }) {
    return <ul>
        {items.map((item, index) => render(item))}
    </ul>
}
